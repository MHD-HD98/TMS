...
class ClockBasedTimer:
    ...
    def log_detection_times(self, tracker_id: int, start_time: datetime, end_time: datetime) -> None:
    ...
    def tick(self, detections: sv.Detections) -> np.ndarray:
        ...
        for tracker_id in detected_tracker_ids:
            if tracker_id not in self.tracker_id2start_time:
                ...
                formatted_start_time = current_time.strftime('%Y-%m-%dT%H:%M:%S')
                self.push_event(current_time, "PBB_FWD_ON")
                print(f"Start time for tracker {tracker_id}: {formatted_start_time}")
        ...
        for tracker_id in list(self.tracker_id2start_time.keys()):
            if tracker_id not in detected_tracker_ids:
                ...
                self.push_event(end_time, "PBB_FWD_OFF")
                print(f"End time for tracker {tracker_id}: {formatted_end_time}")
                duration = (end_time - start_time).total_seconds()
                print(f"Duration for tracker {tracker_id}: {duration:.2f} seconds")
